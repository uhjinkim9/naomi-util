...
function parseEntity(source: string): ParsedEntity | null {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		const propMatch = line.match(
			/^(?:public\s+|private\s+|readonly\s+)?([A-Za-z_][A-Za-z0-9_]*)\??:\s*([^;]+);/
		);
		if (propMatch) {
...
function isRelation(field: Field) {
	return field.decorators.some((d) =>
		/@(OneToOne|OneToMany|ManyToOne|ManyToMany|JoinColumn|JoinTable)\b/.test(
			d
		)
	);
}
...
function isTinyIntBoolean(field: Field) {
	...
	return (
		/@Column\(/.test(meta) &&
		/type\s*:\s*['\"]tinyint['\"]/i.test(meta) &&
		/width\s*:\s*1\b/.test(meta)
	);
}
...
function getRequestValidators(field: Field) {
	const decos: string[] = [];
	...
	if (/Enum\b/.test(t)) {
		...
		if (/type\s*:\s*['\"][a-z]*int['\"]/i.test(columnMeta))
			decos.push("@IsInt()");
		else decos.push("@IsInt()");
	}

	// In request DTO, all fields are optional by policy
...
function genDocDto(
  ...
  lines.push(
	...
	for (const f of fields) {
		...
		const isDateType =
			t === "Date" ||
			/type\s*:\s*['\"][a-z]*date/.test(f.rawColumnMeta || "");
		if (isTiny)
			lines.push(
		...
		// Validators (all fields optional)
		for (const d of getRequestValidators(f)) {
			lines.push(`  ${d}`);
		}
	...
	lines.push(
		"",
		`export class ${parsed.baseName}ReqDto extends PartialType(${className}) {}`
	);
...
export default function EntityToDtoPage() {
	...
	const parsed = useMemo(
		() => (input.trim() ? parseEntity(input) : null),
		[input]
	);
	const dtoCode = useMemo(
		() => (parsed ? genDocDto(input, parsed, {stripAudit}) : ""),
		[parsed, stripAudit, input]
	);
	const resDtoCode = useMemo(
		...
		<div style={{padding: 16, display: "grid", gap: 16}}>
			...
			>
				<div>
					...
					>
						<h3 style={{margin: 0}}>Request DTO</h3>
						<button
							onClick={() =>
				...
				<div>
					...
					>
						<h3 style={{margin: 0}}>Response DTO</h3>
						<button
							onClick={() =>
								navigator.clipboard.writeText(resDtoCode)
							}
							disabled={!resDtoCode}
						>
							Copy
			...
			{!parsed && input.trim() && (
				<div style={{color: "#a00"}}>
					Could not parse entity class name. Ensure it uses `export
					class NameEntity`.
				</div>
			)}
			{!input.trim() && (
				<div style={{color: "#666"}}>
					Paste your entity above. I’ll generate DTOs here.
				</div>
			)}
		</div>
	);
